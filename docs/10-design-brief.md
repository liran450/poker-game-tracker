...
| Component | Notes |
|---|---|
| `<Money>` | The most important component in the app. Signed, LTR-isolated, tabular, sized variants, positive/negative treatment |
| Player row | Active / settled / late-joiner / pending-sync states |
| Buy counter | `−  n  +`, asymmetric target sizes, count-up animation, disabled at 0 |
| Undo snackbar | Single-row form and the upgraded multi-row batch form, with countdown ring |
| Pot banner | Balanced (compact green) and mismatch (expanded red, with actions) |
| Bottom sheet | Base container for action sheet, settle sheet, chip picker, share sheet |
...
| Transfer row | Read mode and edit mode (two name buttons + amount field + delete) |
...
| Connection strip | Offline / pending / synced |
...
- Shared link that has been revoked or expired
- Viewer's read-only game page, signed in vs. anonymous
