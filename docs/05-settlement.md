...
Per game: `buy_amount` B (agorot), `chips_per_buy` C, so **chip value** = B / C.
Example: B = ₪50, C = 100 → each chip is ₪0.5 (#13).

Per player p:

```
owed(p)     = buys(p) × B + customBuys(p)      // what they must put into the game
cashPaid(p) = cash physically handed to the pot at the table          (#18)
cashOut(p)  = chips(p) × B / C                 // what their chips are worth
```

Two different numbers come out of this, and conflating them is the bug that would break the app:

```
net(p)        = cashOut(p) − owed(p)                  ← for STATISTICS. Did they win or lose?
balance(p)    = cashOut(p) − owed(p) + cashPaid(p)    ← for SETTLEMENT. What still has to move?
```

Cash already handed over is an obligation already discharged, so it is added back when working
out who still has to pay whom. It must **not** be added back when asking whether the player had
a good night.
...
That is, the pot is a debtor: it owes P out to the room. The bookkeeping then closes exactly:

```
Σ balance(p) + balance(pot)
  = Σ cashOut − Σ owed + Σ cashPaid − P
  = Σ cashOut − Σ owed              (the cash terms cancel)
...
`קופה → דנה — ₪120`. Requirement #18 satisfied with no extra code path.
...
One transfer, `קופה → רני — ₪60`, and ₪40 of his cash stays in the pot to pay the winners.
...
1. **Fix the counts** — jump to the list of settled players with their chip counts, sorted by
   most recently entered, for a quick re-check.
2. **Assign to the house** — `unaccounted_agorot` absorbs the difference. It becomes a node in the
   settlement graph so the math closes, and is tracked as a long-term statistic.
3. **Split evenly among players** — an alternative some tables prefer. Offer it, don't default
   to it.

Ending the game with a red banner is allowed but requires an explicit, separate confirmation —
the discrepancy must be a decision, never an accident.

## Rounding and precision

- All arithmetic in **integer agorot**. Never floats, never `parseFloat` on user input.
- `cashOut = chips × B / C` can be fractional in agorot (e.g. 7 chips × ₪0.333). Round
  half-to-even per player, then push the accumulated rounding residue onto the largest
  `|balance|` so the totals still sum to zero. Test this explicitly.
- Optionally let the host set a **transfer rounding** of ₪1 or ₪5 ("nobody Bits ₪37.50"). If
  enabled, round each transfer and assign the residue to the biggest winner, and show it:
...
transfer, both settled. This is common in practice (two people who each bought in once, one
doubled up) and it removes them from the harder problem below.
...
that each sum to zero. Finding `k` is NP-hard in general, but a home poker table has at most
~12 nodes, which is trivially small for a bitmask DP:
...
hand, and the difference from optimal is rare and small. Your worked example produces exactly the
three transfers you described.
...
2. Fewer transfers *per person* — nobody should have to send four separate Bits.
3. Rounder numbers.
...
2. **Results section** — every player with `net` and their settlement balance. Collapsible.
...
- **Bit has no public, documented deep-link scheme** for "request ₪50 from this phone number".
  Their URL scheme is not an open API, and anything found by reverse-engineering will break
  without warning and can't be relied on for an app your friends depend on.
- **PayBox** is likewise closed for arbitrary third-party payment intents.

What genuinely works, today, on both platforms, and covers ~95% of the actual benefit:
...
1. **`wa.me` links.** If a player has a phone number on their profile, generate
   ...
   `מור, מהפוקר של אתמול: ₪50 🙏`. One tap from the settlement screen straight into that
   person's WhatsApp chat. This is what people actually do anyway.
...
4. **Mark as paid** — a checkbox per transfer so the host can track who has actually sent it,
   which also feeds a "settles up reliably" statistic. Arguably more useful than a deep link.
